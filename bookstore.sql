...
SELECT CONCAT(author.first_name, ' ', author.last_name)  AS name,             -- Concatenate first and last name
       YEAR(CURDATE()) - YEAR(author.birth_date)         AS age,              -- Calculate age
       COUNT(book.isbn)                                  AS book_title_count, -- Count the number of books
       CONCAT(SUM(book.price * inventory.amount), ' kr') AS inventory_value   -- Calculate total inventory value
FROM author
         JOIN
